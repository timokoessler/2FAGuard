namespace TOTPTokenGuard.Core.Icons
{
    class IconManager
    {
        ...
        public static string GetIcon(string name, IconColor color)
        {
            if (name == null || name == "default")
            {
                return defaultIcon;
            }
            SimpleIcon? icon = SimpleIconsManager.GetIcon(name);
            ...
            if (color == IconColor.Colored && icon.H != null)
            {
                hexColor = $"#{icon.H}";
            }
            ...
            return $"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\" fill=\"{hexColor}\"><path d=\"{icon.P}\"/></svg>";
        }
    }
}
