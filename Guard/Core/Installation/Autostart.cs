...
namespace Guard.Core.Installation
{
    internal class Autostart
    {
            ...
            if (installationType == InstallationType.MICROSOFT_STORE)
            {
                ...
                switch (status.State)
                {
                    case StartupTaskState.Disabled:
                        ...
                        {
                            throw new Exception("Failed to enable startup task: " + result.ToString());
                        }
                        break;
                    case StartupTaskState.DisabledByPolicy:
                        throw new Exception(I18n.GetString("settings.autostart.error.disabled.policy"));
                    case StartupTaskState.DisabledByUser:
                        throw new Exception(I18n.GetString("settings.autostart.error.disabled.user"));
                    case StartupTaskState.Enabled:
                        throw new Exception(I18n.GetString("i.settings.autostart.error.alreadyenabled"));
        ...
        internal static async Task Disable()
        {
            ...
            {
                var status = await StartupTask.GetAsync(PackagedTaskId);
                if (status.State != StartupTaskState.Enabled && status.State != StartupTaskState.EnabledByPolicy)
                {
                ...
                {
                    throw new Exception("Failed to disable startup task: " + status.State.ToString());
                }
                return;
            }
            RegistryKey? rkApp =
        ...
        internal static async Task<bool> IsEnabled()
        {
            ...
            {
                var status = await StartupTask.GetAsync(PackagedTaskId);
                return status.State == StartupTaskState.Enabled || status.State == StartupTaskState.EnabledByPolicy;
            }
